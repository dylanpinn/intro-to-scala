...
object EitherExercises {
  ...
  def getName(providedName: String): Either[AppError, String] = {
    if (providedName.isEmpty) Left(EmptyName)
  ...
  def getAge(providedAge: String): Either[AppError, Int] =
    try {
      ...
      case e: NumberFormatException => Left(InvalidAgeValue(providedAge))
    }
  ...
  def createPerson2(name: String, age: String): Either[AppError, Person] = {
    getName(name).flatMap(validName =>
      getAge(age).map(validAge => Person(validName, validAge))
    )
  }
  ...
  def makeNameUpperCase(name: String, age: String): Either[AppError, Person] = {
    createPerson(name, age).map(p => p.copy(name = p.name.toUpperCase))
  }
  ...
  def createPersonAndShow(name: String, age: String): String = {
    createPerson(name, age) match {
      ...
      case Left(e: InvalidAgeValue) => s"Invalid age value supplied: ${e.value}"
      case Left(e: InvalidAgeRange) =>
        s"Provided age must be between 1-120: ${e.age}"
