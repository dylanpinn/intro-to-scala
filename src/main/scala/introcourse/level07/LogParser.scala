...
object LogParser {
  ...
  case class KnownLog(logLevel: LogLevel, timestamp: Timestamp, message: String)
      extends LogMessage
    ...
    val optLog: Option[LogMessage] = fields match {
      case List("I", timestampStr, message) =>
        parseIntOption(timestampStr).map(timestamp =>
          KnownLog(Info, timestamp, message)
        )
      case List("W", timestampStr, message) =>
        parseIntOption(timestampStr).map(timestamp =>
          KnownLog(Warning, timestamp, message)
        )
  ...
  def parseLogFile(fileContent: String): List[LogMessage] = {
    ...
    lines.filter(line => line != "").map(parseLog(_))
  }
  ...
  /**
    ...
    * = List()
    **/
  def getErrorsOverSeverity(
      logs: List[LogMessage],
      minimumSeverity: Int
  ): List[LogMessage] = {
    logs.filter(log => {
      log match {
        case KnownLog(Error(sev), _, _) if (sev > minimumSeverity) =>
          true
        case _ => false
      }
    })
  ...
  def showLogMessage(log: LogMessage): String = {
    log match {
      case KnownLog(Info, timestamp, msg)    => s"Info ($timestamp) $msg"
      case KnownLog(Warning, timestamp, msg) => s"Warning ($timestamp) $msg"
      case KnownLog(Error(sev), timestamp, msg) =>
        s"Error $sev ($timestamp) $msg"
  ...
  def showErrorsOverSeverity(fileContent: String, severity: Int): List[String] = {
    val parsedFile = parseLogFile(fileContent)
    val errorsOverSev = getErrorsOverSeverity(parsedFile, severity)
    errorsOverSev.map(showLogMessage(_))
  }
